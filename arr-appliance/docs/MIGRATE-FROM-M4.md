# Migrate *arr from iamfaulty-mini (M4) to pawn-shop Mac Mini

Your live stack today (from [truth.iamfaulty.com](https://truth.iamfaulty.com) and `iamfaulty-homelab`):

| What | Where today | Where after migration |
|------|-------------|----------------------|
| NAS | ILLMATIC `192.168.68.69`, share `homelab` | Same — pawn-shop Mini mounts `/mnt/nas` |
| Media | `/Volumes/homelab/media/Movies`, `Shows` | `/mnt/nas/media/Movies`, `Shows` |
| *arr config | `~/homelab-data/arr/` on M4 SSD | `/mnt/nas/personal/arr-appliance/` on NAS |
| Downloads | M4 local SSD (disk drift) | `/mnt/nas/media/downloads` on NAS |
| Jellyfin, NPM, etc. | Stay on M4 | Unchanged |
...
## 2. Copy existing configs from M4 to NAS

On **iamfaulty-mini** (M4), with NAS mounted:
...
# bazarr if you add it fresh or copy from M4 if present
```
...
## 4. Stop *arr on M4 (stop the drift)
...
Keep Jellyfin on M4 — it already reads `/Volumes/homelab/media`.

## 5. VPN note (Gluetun)

Your M4 stack routes qBittorrent through **Gluetun** (VPN kill switch). The pawn-shop `arr-appliance` compose does **not** include Gluetun yet. Options:
...
- Keep qBit on M4 only (defeats the drift goal)
...
`sonarr.iamfaulty.com`, `radarr.iamfaulty.com`, etc. currently point at M4 via Caddy/NPM. After cutover, repoint proxy targets to the **pawn-shop Mini's LAN IP** (or add Tailscale).
