...
for f in fstab.example fstab.smb-nfs.example fstab.iamfaulty.example fstab.synology-ds223j.example env.iamfaulty.example netplan-usb-ethernet.yaml.example arr-appliance.service arr-disk-guard.service arr-disk-guard.timer; do
  copy_file "config/${f}"
done
...
for doc in MORNING-CHECKLIST.md MIGRATE.md HARDWARE.md SYNOLOGY-DS223J.md NOTE-dual-projects.md ROADMAP.md; do
