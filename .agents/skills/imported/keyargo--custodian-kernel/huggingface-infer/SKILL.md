...
Set the required environment variables before use. Until configured, `custodian tools run huggingface-infer` returns a stub response indicating which variables are needed.

## Usage

```bash
custodian tools run huggingface-infer --param value
```
