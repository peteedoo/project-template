---
name: email-send
description: "Send email via SMTP or SendGrid with subject and body"
version: 1.0.0
author: custodian
license: MIT
...
Set the required environment variables before use. Until configured, `custodian tools run email-send` returns a stub response indicating which variables are needed.

## Usage

```bash
custodian tools run email-send --param value
```
