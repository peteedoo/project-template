---
name: cron-list
description: "List all scheduled cron tasks managed by Custodian"
version: 1.0.0
author: custodian
license: MIT
...
Set the required environment variables before use. Until configured, `custodian tools run cron-list` returns a stub response indicating which variables are needed.

## Usage

```bash
custodian tools run cron-list --param value
```
