...
def main():
    ...
    a = p.parse_args()
    try:
        r = requests.get("https://html.duckduckgo.com/html/", params={"q":a.query+" news"}, headers={"User-Agent":"Mozilla/5.0"}, timeout=12)
        parser = DDGParser(); parser.feed(r.text)
        print(json.dumps({"ok":True,"tool":"news-search","query":a.query,"results":parser.results[:a.limit],"count":len(parser.results[:a.limit])}))
    except Exception as e: print(json.dumps({"ok":False,"tool":"news-search","error":str(e)}))
