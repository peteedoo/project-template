---
name: sms-send
description: "Send SMS via Twilio to any phone number"
version: 1.0.0
author: custodian
license: MIT
...
Set the required environment variables before use. Until configured, `custodian tools run sms-send` returns a stub response indicating which variables are needed.

## Usage

```bash
custodian tools run sms-send --param value
```
