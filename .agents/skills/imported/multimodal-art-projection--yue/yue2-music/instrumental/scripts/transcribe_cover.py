...
def main():
    ...
    release = json.loads((Path(__file__).resolve().parents[1] / "assets/cover-release.json").read_text())
    parser.add_argument("--model", default=release["transcriber"]["repo"])
    parser.add_argument("--revision", default=release["transcriber"]["revision"], help="Pin model and remote code to the same commit")
